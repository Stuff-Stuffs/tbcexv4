...
public record BattlePos(int x, int y, int z) {
    ...
    public static final Codec<BattlePos> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Codec.intRange(MIN, MAX).fieldOf("x").forGetter(BattlePos::x),
            Codec.intRange(MIN, MAX).fieldOf("y").forGetter(BattlePos::y),
            Codec.intRange(MIN, MAX).fieldOf("z").forGetter(BattlePos::z)
    ).apply(
            instance,
            BattlePos::new
    ));
