...
public class ServerBattleEnvironmentImpl extends AbstractBattleEnvironmentImpl {
    ...
    protected void setBiome0(final int x, final int y, final int z, final Biome biome) {
        ...
        final RegistryEntry<Biome> entry = world.getRegistryManager().get(RegistryKeys.BIOME).getEntry(biome);
    ...
    protected Biome getBiome0(final int x, final int y, final int z) {
        scratch.set(battle.worldX(x), battle.worldY(y), battle.worldZ(z));
        return ((ServerBattleImpl) battle).world().getChunk(scratch).getBiomeForNoiseGen(BiomeCoords.fromBlock(scratch.getX()), BiomeCoords.fromBlock(scratch.getY()), BiomeCoords.fromBlock(scratch.getZ())).value();
    }
}
